    ...
    <div class="row">
      <div v-for="vault in vaults">
        <div class="col-xs-6 col-sm-2">
          <router-link :to="'/vaults/'+vault._id"><button class="btn vault">{{vault.name}}</button></router-link>
        </div>
      </div>
    </div>
    <div class="row">
        ...
        <h3>Create a new vault</h3>
          <form @submit.prevent="createVault">
            <div class="form-group">
              <input type="text" class="form-control" v-model="name" placeholder="Vault Name" required>
            </div>
            <div class="form-group">
...
export default {
  ...
  methods:{
    createVault(){
      this.$store.dispatch('createVault', {name: this.name, description: this.description, creatorId: this.creatorId})
    },
    logout() {
